...
public class VFSTest {
  ...
  public void testMounting() throws IOException {
    File dir = null;
    try {
      dir = File.createTempFile("vfs-test", ".dir");
      dir.delete();
      dir.mkdirs();
      IOHelper.write(new File(dir, "file1"), "file1");
      final File sub = new File(dir, "sub");
      sub.mkdir();
      IOHelper.write(new File(sub, "file2"), "file2");
  ...
  public void testMountAndWrite() throws IOException {
    ...
    try {
      ...
      assertThat(IOHelper.readToString(new File(dir, "file1")), is("file1"));
    } finally {
      if (dir != null) {
        IOHelper.deleteDirectory(dir);
      }
  ...
  public void testExportFS() throws IOException {
    File target = null;
    try {
      target = File.createTempFile("vfs-test", ".dir");
      target.delete();
      target.mkdirs();
      ...
      assertThat(IOHelper.readToString(new File(target, "dir1/a.txt")),
          is("a.txt"));
      assertThat(IOHelper.readToString(new File(target, "dir1/b.txt")),
          is("b.txt"));
      assertThat(IOHelper.readToString(new File(target, "dir1/sub/c.txt")),
          is("c.txt"));
      assertThat(IOHelper.readToString(new File(target, "dir2/d.txt")),
          is("d.txt"));
      assertThat(IOHelper.readToString(new File(target, "dir2/e.txt")),
          is("e.txt"));
    } finally {
      if (target != null) {
        IOHelper.deleteDirectory(target);
      }
    }
  }
  ...
  public void testImportFS() throws IOException {
    File source = null;
    try {
      source = File.createTempFile("vfs-test", ".dir");
      source.delete();
      source.mkdirs();
      IOHelper.write(new File(source, "dir1/a.txt"), "a.txt");
      IOHelper.write(new File(source, "dir1/b.txt"), "b.txt");
      IOHelper.write(new File(source, "dir1/sub/c.txt"), "c.txt");
      IOHelper.write(new File(source, "dir2/d.txt"), "d.txt");
      IOHelper.write(new File(source, "dir2/e.txt"), "e.txt");
      ...
      if (source != null) {
        IOHelper.deleteDirectory(source);
      }
    }
  }
